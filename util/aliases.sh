...
externals() {
    ...
    for host in localhost c-jgraehl c-ydong c-mdreyer; do
...
linjen() {
    ...
    (set -e;
        ...
        cjg macget $branch
        cjg nightly=$nightly threads=$threads VERBOSE=${VERBOSE:-0} jen "$@" 2>&1) | tee ~/tmp/linjen.$branch | filter-gcc-errors
}
jen() {
    cd $xmtx
