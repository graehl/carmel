...
buildclangformat() {
    ...
    cp -a llvm-build/lib/libclang.so* $prefix/bin
}
...
errors() {
    if [[ $* ]] ; then
        egrep --color=always -B 2 -A 1 'fatal|FATAL|Fatal|Error|ERROR|error|FAIL|failed|exit code' "$@"
    elif [[ `echo *` ]] ; then
        errors * condor/* `exclude_suffix .condor condor/*/*`
    fi
}
previews() {
    if [[ ! $* ]] ; then
...
gccsysincludes() {
    echo | ${1:-g++} -Wp,-v -x c++ - -fsyntax-only 2>&1 | perl -ne '
chomp;
if (s/^\s//) {
   print STDERR "\nls -d $_ # WARNING no such directory\n" if (! -d $_);
   print " -I$_"
}
'
}
...
if ! inpath cmake; then
    extcmake=$xmtext/tools/cmake-3.9
    if [[ -x $extcmake/bin/cmake ]] ; then
        export PATH=$extcmake/bin:$PATH
    fi
fi
untidyclang() {
    local f=$1
    shift
    local g=${f#/Users/graehl}
    if [[ $g != $f ]] ; then
        f=/home/graehl/$g
    fi
    if [[ -f $f ]] ; then
    ...
    perl -i~ -pe "unless (/\Q$1\E/) { s/(Checks: ')/\\1$1,/;s/,'$/'/ }"  $f
    git add $f
    cat $f
    realpath $f
    echo
