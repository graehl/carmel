...
buildcar() {
    ...
    [ "$noclean" ] || make clean
    set -x
    make CMDCXXFLAGS+="-I$FIRST_PREFIX/include" LDFLAGS+="-lpthread -L$FIRST_PREFIX/lib" BOOST_SUFFIX= -j 4
    make CMDCXXFLAGS+="-I$FIRST_PREFIX/include" LDFLAGS+="-L$FIRST_PREFIX/lib" BOOST_SUFFIX= install
    set +x
    popd
    if [ "$1" ] ; then
...
clonecar() {
    set -x
    git config svn.authorsfile $SVNAUTHORS && git svn --authors-file=$SVNAUTHORS clone --username=graehl --ignore-paths='^(NOTES.*|cipher|clm|gextract|syscom|tt|xrsmodels|Jamfile)' https://nlg0.isi.edu/svn/sbmt/trunk/graehl --no-metadata
    set +x
}
