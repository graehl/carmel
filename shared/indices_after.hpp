...
// o[i]=j where v[i] before remove_marked(v,marked) == v[j] after.  o[i]=-1 if marked[i]. returns size after
// removing (max o[i]+1)
template <class O>
...
// outputs sequence to out iterator, of new indices for each element i, corresponding to deleting element i
// from an array when remove[i] is true (-1 if it was deleted, new index otherwise), returning one-past-end of
// out (the return value = # of elements left after deletion)
template <class REMOVE, class REMOVEe, class O>
unsigned new_indices(REMOVE i, REMOVEe end, O out, bool remove = true) {
  unsigned f = 0;
  while (i != end) *out++ = *i++ ? (unsigned)-1 : f++;
  return f;
};
...
struct iter_not {
  ...
  void operator++() { ++i; }
  bool operator==(self_type const& o) { return i == o.i; }
};
...
unsigned new_indices_keep(KEEP i, KEEPe end, O out, bool means_keep = true) {
  unsigned f = 0;
  while (i != end) *out++ = (*i++ == means_keep) ? f++ : (unsigned)-1;
  return f;
};
...
// to cannot be same as from, for most permutations.  for to==from, use indices_after::init_inverse_order
// instead.
...
// given a vector and a parallel sequence of bools where true means keep, keep only the marked elements while
// maintaining order.
// this is done with a parallel sequence to the input, marked with positions the kept items would map into in
// a destination array, with removed items marked with the index -1.  the reverse would be more compact
// (parallel to destination array, index of input item that goes into it) but would require the input sequence
// be random access.
struct indices_after {
  ...
  void init_alloc(unsigned n) {
    free();
    n_mapped = n;
    map = n_mapped > 0 ? (unsigned*)::operator new(sizeof(unsigned) * n_mapped) : 0;
  }
  void init_const(unsigned n, unsigned map_all_to) {
    init_alloc(n);
    for (unsigned i = 0; i < n; ++i) map[i] = map_all_to;
  ...
  void free() {
    if (n_mapped) ::operator delete((void*)map);
    n_mapped = 0;
  }

  bool removing(unsigned i) const { return map[i] == REMOVED; }
  bool keeping(unsigned i) const { return map[i] != REMOVED; }

  unsigned operator[](unsigned i) const { return map[i]; }
 ...
 private:
  indices_after(self_type const&) { map = NULL; }
};
