...
/** \file
    ...
    only for types where you can use memcpy to move/swap (thus, also more
    efficient).

    FIXME: const safeness for contents e.g. a[1] a.at(1) return const ref if a
    const?

    For (MEMCPY-able-to-move) types *only* (e.g. plain old data)
    constructor/destructor that do things is fine, as long as nothing goes wrong
    when relocating the object to a diff address by just moving the bytes with no
    notification, e.g. circularly linked head node as a member will fail badly
    Array encapsulates a region of memory and doesn't own its own storage ... you
    can create subarrays of arrays that use the same storage.  you could
    implement vector or string on top of it.  it does take an allocator argument
    and has methods alloc, dealloc, re_alloc (realloc is a macro in MS, for
    shame), which need not be used.  as a rule, nothing writing an Array ever
    deallocs old space automatically, since an Array might not have alloced it.
    dynamic_array extends an array to allow it to be grown efficiently one
    element at a time (there is more storage than is used) ... like vector but
    better for MEMCPY-able stuff note that dynamic_array may have capacity()==0
    (though using push_back and array(i) will allocate it as necessary)
*/
...
class array : protected Alloc {
 ...
 public:
  friend std::size_t hash_value(array const& x) { return boost_hash_range(x.begin(), x.end()); }
  static inline size_type bytes(size_type n) { return sizeof(T) * n; }
  ...
  //    BOOST_STATIC_CONSTANT(bool,DUMMY=0); // msvc++ insists on amibuity between template Writer print and
  //    bool 2nd arg ...

  T& at(unsigned index) const {  // run-time bounds-checked
    T* r = vec + index;
    if (!(r < end()))
      throw std::out_of_range(std::string("array access out of bounds with index=")
                                  .append(boost::lexical_cast<std::string>(index)));
    return *r;
  }
  bool exists(unsigned index) const { return begin() + index < end(); }
  ...
  bool invariant() const { return vec >= endspace; }
  ...
  array<T, Alloc> substr(unsigned start) const { return substr(start, size()); }
  ...
  void construct() {
    for (T* p = vec; p != endspace; ++p) new (p) T();
  }
  template <class V>
  void construct(const V& val) {
    for (T* p = vec; p != endspace; ++p) new (p) T(val);
  }
  ...
  std::basic_ostream<charT, Traits>& print(std::basic_ostream<charT, Traits>& o, bool multiline = false,
                                           bool dummy = false, bool dummy2 = false) const {
  ...
  std::basic_ostream<charT, Traits>& print(std::basic_ostream<charT, Traits>& o, Writer w,
  ...
  template <class T2, class Alloc2, class charT, class Traits, class Reader>
  friend std::ios_base::iostate read_imp(array<T2, Alloc2>* s, std::basic_istream<charT, Traits>& in,
                                         Reader read, unsigned reserve);
  template <class L, class A>
  friend void read(std::istream& in, array<L, A>& x, StackAlloc& a);  // throw(genio_exception);
  ...
  bool empty() const { return vec == endspace; }
  ...
  void destroy() {
    for (T* i = begin(); i != end(); ++i) i->~T();
  }

  void copyto(T* to, unsigned n) { std::memcpy(to, vec, bytes(n)); }

  void copyto(T* to) { copyto(to, capacity()); }

  void release_impl() { vec = endspace = 0; }
  ...
  array(const array<T, Alloc2>& o)
      : vec(o.vec), endspace(o.endspace) {}
  ...
  explicit array(const char* c) { std::istringstream(c) >> *this; }

  // create array reference to a STL vector - note: I *believe* stdc++ requires storage be contiguous, i.e.
  // iterators be pointers ... if not, nonportable
  explicit array(std::vector<T>& vec) { init(vec); }
  ...
  template <class A2>
  array(typename std::vector<T, A2>::const_iterator begin, typename std::vector<T, A2>::const_iterator end)
      : vec(const_cast<T*>(&*begin)), endspace(const_cast<T*>(&*end)) {}
  ...
  const T* end() const { return endspace; }

  const_reverse_iterator rbegin() const { return const_reverse_iterator(end()); }
  const_reverse_iterator rend() const { return const_reverse_iterator(begin()); }

  const T* const_begin() const { return vec; }
  const T* const_end() const { return endspace; }
...
/** array that owns it buffer.

    questionable: if copied, doesn't copy contents. this should have a non-copy-ctor interface
*/
template <typename T, typename Alloc = std::allocator<T> >
class auto_array : public array<T, Alloc> {
 public:
  typedef array<T, Alloc> Super;
  explicit auto_array(unsigned sp = 0) : Super(sp) {}
  ~auto_array() { this->dealloc(); }
  void clear() { this->dealloc(); }

  void alloc(unsigned sp) { Super::re_alloc(sp); }
  template <class charT, class Traits, class Reader>
 ...
 private:
  void operator=(const array<T, Alloc>& a) { dynarray_assert(0); }
...
/**
   like dynamic_array (std::vector) but can't adjust size
*/
template <typename T, typename Alloc = std::allocator<T> >
class fixed_array : public auto_array<T, Alloc> {
 public:
  ...
  typedef unsigned size_type;
  explicit fixed_array(size_type sp) : Super(sp) { this->construct(); }
  fixed_array(T const& t, size_type sp) : Super(sp) { this->construct(t); }
  ~fixed_array() { this->destroy(); }
  void clear() {
    this->destroy();
    this->dealloc();
  }
  ...
  template <class A>
  fixed_array(const dynamic_array<T, A>& a)
      : Super(a.capacity()) {
    // DBPC("fixed_array copy",a);
    uninit_copy_from(a.begin(), a.end());
  }
  ...
  // having problems with ambiguous constructor?  make sure you pass an *unsigned* int if you want to specify
  // # of elements.  I don't know how to make this constructor exclude integral types, well, I could do it
  // with type traits and a second param is_container<F> *dummy=0
  /*
    template <class F>
...
// FIXME: overriding std::swap is technically illegal, but this allows some dumb std::sort to swap, which it
// should have found in graehl::swap by arg. dependent lookup.
namespace std {
template <class T, class A>
