...
/// surround with quotes and \" \\ only if any character inside is special \ or ".  note:
/// is_special(whitespace) should return true if you're going to use i >> std::string. but if you use
/// in_quote, it need not.
...
inline void out_quote(std::basic_ostream<Ch, Tr>& out, const C& data, IsSpecial is_special = IsSpecial(),
  ...
  if (quote) {
    ...
    for (i_iter i(s); i != end; ++i) {
      Ch c = *i;
      if (c == quote_char || c == escape_char) out.put(escape_char);
      out.put(c);
    }
    out << quote_char;
  } else {
    ...
    */
    Ch c;
    while (s.get(c)) out.put(c);
  }
}

template <class C, class Ch, class Tr>
inline void out_quote(std::basic_ostream<Ch, Tr>& out, const C& data, char quote_char = '"',
                      char escape_char = '\\') {
  out_quote(out, data, false_for_all_chars(), quote_char, escape_char);
}

template <class Output, class Ch, class Tr, class IsSpecial>
inline Output in_quote(std::basic_istream<Ch, Tr>& i, Output out, IsSpecial is_special = IsSpecial(),
                       char quote_char = '"', char escape_char = '\\') {
  char c;
  if (i.get(c)) {
    *out++ = c;
  ...
  }  // else end of stream.  shouldn't interpret as empty token but not throwing exception either.  check
     // status of i before you use.
  return out;
fail:
  throw std::runtime_error("end of file reached when parsing quoted string (in_quote)");
}

template <class Ch, class Tr, class IsSpecial>
inline std::string in_quote(std::basic_istream<Ch, Tr>& i, IsSpecial is_special = IsSpecial(),
                            char quote_char = '"', char escape_char = '\\') {
  //    std::basic_stringstream<Ch,Tr> s;
  std::vector<Ch> v;
...
inline Output copy_escaping(Input in, Input in_end, Output out, SpecialChar is_special,
                            char escape_char = '\\') {
  or_true_for_char<SpecialChar> need_quote(escape_char, is_special);
  for (; in != in_end; ++in) {
    char c = *in;
    if (need_quote(c)) *out++ = escape_char;
    *out = c;
    ++out;
  }
  return out;
...
struct printed_stringstream : public std::basic_stringstream<Ch, Tr> {
  ...
  stream& as_stream() { return *(stream*)this; }
  const stream& as_stream() const { return *(const stream*)this; }
...
inline Output print_escaping(const Data& data, Output out, SpecialChar is_special = SpecialChar(),
                             char escape_char = '\\') {
...
inline Output print_quoting_if(QuoteProtChar quote_if, const Data& data, Output out, char quote_char = '"',
                               SpecialChar escape_inside_quotes = SpecialChar(), char escape_char = '\\') {
  printed_stringstream<typename Output::value_type> pr(data);
...
template <class Data, class Output, class SpecialChar>
inline Output print_maybe_quoting(const Data& data, Output out, char quote_char = '"',
                                  SpecialChar is_special = SpecialChar(), char escape_char = '\\') {
  return print_quoting_if(or_true_for_chars<SpecialChar>(escape_char, quote_char, is_special), data, out,
                          quote_char, is_special, escape_char);
}
