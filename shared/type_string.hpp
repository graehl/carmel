...
/* type names for end users. override free function type_string_impl(T) in T's namespace to define T's name
...
template <class T>
std::string type_string_impl(T const&) { return std::string(); }
...
#define GRAEHL_PRIMITIVE_TYPE_STRING(T, name) template <> struct type_string_traits<T> \
...
struct type_string_traits<std::map<std::string, T> > {
  ...
  {
    return "map to "+type_string(*(T const*)0);
  }
};
