...
struct path_traits {
  ...
  static inline bool includes(cost_type const& a,cost_type const& b,float delta=FLOAT_EPSILON) {
    return !updates(repeat(a,1+delta),b); } // just used for debugging so far - should work for Log or Viterbi
  static inline bool close_enough(cost_type const& a,cost_type const& b,float delta=FLOAT_EPSILON) { return includes(a,b,delta)&&includes(b,a,delta); } // also for debugging
};
