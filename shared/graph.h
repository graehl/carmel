...
// simple adjacency graph with path-additive weight/cost (lower is better, 0 is no cost, +INF is infinite
// cost)
...
struct GraphArc {
  ...
  Weight& wt() const { return *(Weight*)&weight; }
  /*
...
#include <graehl/shared/warning_push.h>
  GCC_DIAG_IGNORE(strict-aliasing)
  template <class T>
  T& data_as() {
    return *(T*)(&data);
  }
  /// note: not safe to use with integral types if you construct as void * data ... only safe if you set with
  /// data_as()
  template <class T>
...
#include <graehl/shared/warning_pop.h>
  ...
  GraphArc() {}
  GraphArc(unsigned src, unsigned dest, FLOAT_TYPE weight, void* data)
      : src(src), dest(dest), weight(weight), data(data) {}
  GraphArc(unsigned src, unsigned dest, FLOAT_TYPE weight) : src(src), dest(dest), weight(weight) {}
  // private:
...
struct GraphState {
  ...
  void add(GraphArc const& a) { arcs.push(a); }
  std::size_t outdegree() const { return arcs.size(); }

  template <class T>
  void add_data_as(unsigned src, unsigned dest, FLOAT_TYPE weight, T const& data) {
    arcs.push_front(src, dest, weight);
    arcs.front().template data_as<T>() = data;
  }

  void add(unsigned src, unsigned dest, FLOAT_TYPE weight, void* data) {
    arcs.push_front(src, dest, weight, data);
  }

  void add(unsigned src, unsigned dest, FLOAT_TYPE weight) { arcs.push_front(src, dest, weight); }
  template <class W>
...
struct Graph {
  ...
  template <class W>
  void setwt(W const& w) {
    for (unsigned i = 0; i < nStates; ++i) states[i].setwt(w);
  }
  typedef GraphState::saved_weights_t saved_weights_t;
  void save_weights(saved_weights_t& s) const {
    for (unsigned i = 0; i < nStates; ++i) states[i].save_weights(s);
  }
  unsigned restore_weights(saved_weights_t const& s, unsigned start = 0) {
    for (unsigned i = 0; i < nStates; ++i) start = states[i].restore_weights(s, start);
    return start;
  }
};

// take the arcs in graph (src,n_states) and add them, reversing src<->dest, to destination graph
// (destination,n_states).  copy data field unless data_point_to_forward, in which case point to original
// (forward) arc
void add_reversed_arcs(GraphState* destination, GraphState const* src, unsigned n_states,
                       bool data_point_to_forward = false);
...
void depthFirstSearch(Graph graph, unsigned startState, bool* visited,
                      void (*func)(unsigned state, unsigned pred));

template <class Weight>
void countNoCyclePaths(Graph g, Weight* nPaths, unsigned src);
...
struct backrefs {
  ...
  void use(unsigned s) {
    if (ids[s].use(nextid)) order_from(s);
  }
...
class TopoSort {
  typedef std::front_insert_iterator<List<unsigned> > UnsignedPusher;
  Graph g;
  bool* done;
  bool* begun;
  UnsignedPusher o;
 ...
 public:
  bool has_cycle() const { return n_back_edges; }
  TopoSort(Graph g_, List<unsigned>* l) : g(g_), o(*l), n_back_edges(0) {
    done = NEW bool[g.nStates];
    begun = NEW bool[g.nStates];
    for (unsigned i = 0; i < g.nStates; ++i) done[i] = begun[i] = false;
  }
  void order_all() { order(true); }
  void order_crucial() { order(false); }
  void order(bool all = true) {
    for (unsigned i = 0; i < g.nStates; ++i)
      if (all || !g.states[i].arcs.empty()) order_from(i);
  }
...
struct reverse_topo_order {
 ...
 public:
  ...
  void order(O o, bool all = true) {
    for (unsigned i = 0; i < g.nStates; ++i)
      if (all || !g.states[i].arcs.empty()) order_from(o, i);
  }
  unsigned get_n_back_edges() const { return n_back_edges; }
  template <class O>
  void order_from(O o, unsigned s) {
    if (done[s]) return;
    if (begun[s]) {  // this state previously started but not finished already = back edge/cycle!
      ++n_back_edges;
      return;
...
// computes best paths from all states to single destination, storing tree of arcs taken in *pathTree, and
// distances to dest in *dist
...
struct rewrite_GraphState {
  ...
  void operator()(GraphState& s) { s.arcs.clear(); }
};

// as removeStates(..), but graph g is modified by swapping deleted states to the end (and clearing their
// arcs), returns number of states remaining
...
template <class Weight>
void countNoCyclePaths(Graph g, Weight* nPaths, unsigned src, unsigned* p_n_back_edges = 0) {
  List<unsigned> topo;

  TopoSort sort(g, &topo);
  sort.order_from(src);
  if (p_n_back_edges) *p_n_back_edges = sort.get_n_back_edges();
  for (unsigned i = 0; i < g.nStates; ++i) nPaths[i] = 0;
  nPaths[src] = 1;
  for (List<unsigned>::const_iterator t = topo.const_begin(), end = topo.const_end(); t != end; ++t) {
    unsigned src = *t;
...
template <class Weight>
Weight countNoCyclePathsTo(Graph g, unsigned src, unsigned dest, unsigned* p_n_back_edges = 0) {
...
void propagate_paths_in_order(Graph g, Order t, Order const& t_order_end, Weight_get const& getwt,
                              Weight_array& w) {
  for (; t != t_order_end; ++t) {
    unsigned src = *t;
    const List<GraphArc>& arcs = g.states[src].arcs;
...
struct get_wt {
  Weight const& operator()(GraphArc const& a) const { return a.wt(); }
};
