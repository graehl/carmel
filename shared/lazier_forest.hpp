...
struct lazy_hyperedge {
  ...
  inline bool operator <(lazy_hyperedge const& o) const { // true if o should dominate max-heap, i.e. o is better than us
  ...
  void print(O &o) const {
    ...
    if (child[0]) {
      ...
      if (child[1])
        o << "," << PointerInt(child[1]) << '[' << childbp[1] << ']';
    }
    o << ")=" << derivation;
    o << '}';
  }
...
struct lazier_forest_options {
  ...
  lazier_forest_options(lazy_kbest_index_type pqReserve=0)
      : throw_on_cycle()
      , pqReserve(pqReserve)
  {}
...
struct lazier_forest_filter_options {
  ...
  lazier_forest_filter_options(std::size_t reserveForestIdMap=1000000,
                               lazy_kbest_index_type pqReserve=0,
                               std::size_t forestSz = sizeof(forest))
      : Base(pqReserve)
      , forestpool(forestSz)
      , forests(reserveForestIdMap)
  {}

  void clear_stats()
  {
    this->stats.clear(FilterFactory::filter_type::trivial);
  }
};

/**
   this is like lazy_forest except you don't build in advance all the lazy_forest nodes. instead your DerivationFactory also provides:
   ...
   bool expand(std::size_t stateid, typename lazier_forest_options<MyDerivFactory>::lazy_hyperedges_queue &edges);

   };

*/
template <class DerivationFactory, class FilterFactory>
struct lazier_forest : FilterFactory::filter_type // empty base class opt. - may have state e.g. hash of seen strings or trees
{
  typedef DerivationFactory derivation_factory_type;
  typedef FilterFactory filter_factory_type;
  ...
  explicit lazier_forest(options const& opt)
      : filter_type(opt.filter.filter_init())
      , pq(opt.pqReserve)
  {}
  ...
  {
    return (derivation_type)DerivationFactory::PENDING();
  }
  ...
  //TODO: faster heap operations if we put handles to hyperedges on heap instead of copying? boost object pool?
  ...
  derivation_type get_best(options &opt, lazy_kbest_index_type n) {
    ...
    if (n < memo.size()) {
      ...
      if (memo[n] == PENDING()) {
        KBESTERRORQ("lazier_forest::get_best","memo entry " << n << " for lazier_forest@0x" << (void*)this << " is pending - there must be a negative cost (or maybe 0-cost) cycle - returning NONE instead (this means that we don't generate any nbest above " << n << " for this node."); //=" << memo[n-1]
        if (opt.throw_on_cycle)
