...
class HashEntry {
 ...
 public:
  ...
  HashEntry(const K& k, const V_init& v)
      : next(NULL), first(k), second(v) {}
  template <class V_init>
  HashEntry(const K& k, const V_init& v, HashEntry<K, V>* const n)
      : next(n), first(k), second(v) {}
...
class HashIter {  // Yaser added this - 7-27-2000
 ...
 public:
  ...
  bool operator==(void* nocare) const { return entry == NULL; }
  const HashEntry<K, V>* operator!=(void* nocare) const { return entry; }
...
template <class K, class V, class H = hash<K>, class P = std::equal_to<K>,
          class A = std::allocator<HashEntry<K, V> > >
class HashTable : private A::template rebind<HashEntry<K, V> >::other {
 public:
  typedef K key_type;
  typedef V mapped_type;
  typedef H hasher;
 ...
 protected:
  unsigned
      siz;  // always a power of 2, stored as 1 less than actual for fast mod operator (implemented as and)
  unsigned cnt;
  ...
  Node** table;
  std::size_t hashToPos(std::size_t hashVal) const { return hashVal & siz; }
 ...
 public:
...
#endif
    for (buckets_iterator i = const_cast<Node**>(table), e = const_cast<Node**>(table + siz); i <= e;
         ++i) {  // <= because siz = #buckets-1
  ...
  class local_iterator : public std::iterator<std::forward_iterator_tag, std::pair<const K, V> > {
   ...
   public:
    ...
    value_type* operator->() const { return (value_type*)m_rep; }
    bool operator==(const local_iterator& x) const { return m_rep == x.m_rep; }
    bool operator!=(const local_iterator& x) const { return m_rep != x.m_rep; }
  };
  typedef local_iterator const_local_iterator;

  const_local_iterator begin(std::size_t i) const { return const_cast<Node*>(table[i]); }
  const_local_iterator end(std::size_t i) const { return NULL; }
  local_iterator begin(std::size_t i) { return table[i]; }
  local_iterator end(std::size_t i) { return NULL; }

  const_iterator begin() const { return ((HashTable<K, V, H, P, A>*)this)->begin(); }
  iterator begin() { return iterator(*this); }
  find_result_type end() const { return NULL; }

  std::size_t bucket(const K& first) const { return hashToPos(get_hash()(first)); }
  std::size_t bucket_size(std::size_t i) {
    std::size_t ret = 0;
    for (local_iterator l = begin(i), e = end(i); l != e; ++l) ++ret;
    return ret;
  }
...
#endif
  {
    ...
    for (unsigned i = 0; i <= siz; ++i) table[i] = clone_bucket(ht.table[i]);
  }


  explicit HashTable(unsigned sz = DEFAULTHASHSIZE, float mLoad = DEFAULTHASHLOAD) { init(sz, mLoad); }
 ...
 protected:
  ...
  void init(unsigned sz = DEFAULTHASHSIZE, float mLoad = DEFAULTHASHLOAD) {
    ...
    if (growAt < 2) growAt = 2;
    siz--;  // size is actually siz + 1
    alloc_table(bucket_count());
    for (unsigned i = 0; i <= siz; i++) table[i] = NULL;
  }
  void grow() { rehash_pow2(2 * bucket_count()); }
 ...
 public:
  ...
  // bool is true if insertion was performed, false if key already existed.  pointer to the key/val pair in
  // the table is returned
 private:
  bool equal(const K& k, const K& k2) const { return get_eq()(k, k2); }
 ...
 public:
  ...
  insert_result_type insert(const K& first, const V& second /*=V()*/) {
    ...
    for (Node* p = table[bucket]; p; p = p->next)
      if (equal(p->first, first)) return insert_result_type((find_result_type)p, false);
    ...
    return insert_result_type(reinterpret_cast<find_result_type>(table[bucket]), true);
  }
  ...
  insert_result_type insert(const value_type& t) { return insert(t.first, t.second); }


  find_result_type find(const K& first) const {
    for (Node* p = table[bucket(first)]; p; p = p->next)
      if (equal(p->first, first)) return reinterpret_cast<find_result_type>(p);
    return NULL;
  }
  V* find_second(const K& first) const {
    for (Node* p = table[bucket(first)]; p; p = p->next)
      if (equal(p->first, first)) return &(p->second);
    return NULL;
  }
  ...
  V const& operator[](const K& first) const { return *find_second(first); }
 ...
 protected:
  void rehash_pow2(unsigned request) {
    ...
    for (i = table; i <= table + siz; i++) *i = NULL;
 ...
 private:
  ...
  void free_table(Node** t, unsigned size) { table_alloc().deallocate(t, size + 1); }
#else

  void alloc_table(unsigned _n) { table = NEW Node * [_n]; }
  void free_table(Node** t, unsigned size) { delete[] t; }
#endif
...
/*
  ...
  template <typename K, typename H=::hash<K>, typename P=std::equal_to<K>, typename
  A=std::allocator<HashEntry<K,V> > > class HashSet : public HashTable<K,V,H,P,A> {
  };
...
/*
  template <class K,class V,class H,class P,class A>
  inline typename HashTable<K,V,H,P,A>::find_result_type *find_value(const HashTable<K,V,H,P,A>& ht,const K&
  first) {
...
struct hash<unsigned> {
  unsigned operator()(unsigned key) const { return uint32_hash(key); }
};

template <>
struct hash<char> {
  char operator()(char key) const { return key; }
};


template <>
struct hash<int> {
  unsigned operator()(int key) const { return uint32_hash(key); }
};

template <>
struct hash<const char*> {
  std::size_t operator()(const char* s) const { return cstr_hash(s); }
};
