...
/** \file
    ...
    like std::vector but exposes contiguous-array-implementation - only for types
    where you can use memcpy to move/swap (thus, also more efficient). works for
    nested dynamic_arrays as long as the bottom level is memcpy move/swappable
    historical justification: when Carmel was first written, STL wasn't supported
    by gcc. TODO: with c++11 std::vector can probably serve at least as well.
*/
...
// caveat:  cannot hold arbitrary types T with self or mutual-pointer refs; only works when memcpy can move
// you
// default Alloc defined in fixed_array.hpp
template <typename T, class Alloc>
class dynamic_array : public array<T, Alloc> {
 public:
 ...
 private:
  void destroy_contents() {
    for (T* i = this->vec, * e = end(); i != e; ++i) i->~T();
  }

  //  dynamic_array& operator = (const dynamic_array &a) {std::cerr << "unauthorized assignment of a dynamic
  //  array\n";dynarray_assert(0);}
 public:
#if __cplusplus >= 201103L
  /// move
  dynamic_array(dynamic_array&& o) noexcept {
    std::memcpy(this, &o, sizeof(dynamic_array));
    o.clear_nodestroy();
  }
  ...
  friend std::size_t hash_value(dynamic_array const& x) { return boost_hash_range(x.begin(), x.end()); }
  ...
  void reinit_nodestroy(size_type sp, const T& t = T()) {
    reserve(sp);
    endv = this->vec + sp;
    for (T* p = this->vec; p != this->endv; ++p) new (p) T(t);
  }
  ...
  // FIXME: test:
  template <class RIT>
  dynamic_array(RIT b, RIT e, size_type extra_pre, size_type extra_post)
      : array<T, Alloc>(e - b + extra_pre + extra_post) {
 ...
 private:
  ...
  void imp_construct_extra(T* b, T* e, size_type extra_pre, size_type extra_post) {
    for (T* be = b + extra_pre; b < be; ++b) new (b) T();
    --e;
 ...
 public:
  ...
  template <class RIT>
  dynamic_array(typename boost::disable_if<boost::is_arithmetic<RIT>, RIT>::type const& a, RIT const& b)
      : array<T, Alloc>(b - a) {
  ...
  void copyto(T* to, T* from, size_type n) { memcpy(to, from, bytes(n)); }
  void copyto(T* to, size_type n) { copyto(to, this->vec, n); }
  void copyto(T* to) { copyto(to, size()); }
  ...
  const T* const_end() const { return endv; }
  const T* cend() const { return endv; }
  ...
  T& at(size_type index) const {  // run-time bounds-checked
    T* r = this->vec + index;
    if (!(r < end())) throw std::out_of_range("dynamic_array index out of bounds");
    return *r;
  }
  bool exists(size_type index) const { return this->vec + index < end(); }
  ...
  // NEW OPERATIONS:
  // like [], but bounds-safe: if past end, expands and default constructs elements between old max element
  // and including new max index
    ...
    if (index >= size()) {
      ...
      if (index >= newSpace) {
        if (newSpace == 0)
          ...
          do {
            newSpace *= 2;
          } while (index >= newSpace);  // FIXME: overflow if newSpace > 2^31
        realloc_up(newSpace);
      }
      T* v = end();
      endv = this->vec + index + 1;
      while (v < endv) new (v++) T();
    }
    return (this->vec)[index];
  }
  void push(const T& it) { push_back(it); }
  T& top() { return back(); }
  void pop_back() {
    (--endv)->~T();
    ;
  ...
  void push_back() { new (push_back_raw()) T(); }
  template <class T0>
  ...
  void push_back_n(const T& val, size_type n) {
    ...
    for (T* p = endv; p != newend; ++p) new (p) T(val);
    endv = newend;
    dynarray_assert(invariant());
  }
  ...
  void push_keeping_front_best(T& t, Better_than_pred better) {
    throw "untested";
    push_back(t);
    if (empty() || better(front(), t)) return;
    swap(front(), back());
  }
  ...
  void removeMarked(bool marked[]) { graehl::remove_marked_swap(*this, marked); }
  ...
  void removeMarked_nodestroy(bool marked[]) {
    ...
    if (i < sz) {
      (this->vec)[i++].~T();
      for (;;) {
        while (i < sz && marked[i]) (this->vec)[i++].~T();
        if (i == sz) break;
        size_type i_base = i;
        while (i < sz && !marked[i]) ++i;
        if (i_base != i) {
 ...
 protected:
  ...
  void dealloc_safe() {
    ...
    if (oldcap) this->deallocate(this->vec, oldcap);  // can't throw
  }

 public:
#if GRAEHL_DYNARRAY_POD_ONLY_SERIALIZE
  // note: using this implies a trivial destructor / POD copy constructor (which resizable dynarray already
  // exploits)
  template <class A>
  void serialize(A& a) {
    size_type sz;
  ...
  */
  ...
  void reserve(size_type newSpace) {
    if (newSpace > this->capacity()) realloc_up(newSpace);
  }
  ...
  void clear_nodestroy() { endv = this->vec; }
  void clear() {
    dynarray_assert(invariant());
    destroy_contents();
    clear_nodestroy();
  ...
  ~dynamic_array() { destroy_contents(); }
  ...
  std::basic_ostream<charT, Traits>& print(std::basic_ostream<charT, Traits>& o, Writer writer,
  ...
  std::ios_base::iostate read(std::basic_istream<charT, Traits>& in, Reader read, bool append = false) {
    ...
    if (c == '(') {
      for (;;) {
        ...
        in.unget();
        push_back();  // was doing push_back_raw, but that's bad - need to default construct some types where
        // reader assumes constructedness.
...
struct plus_one_reader {
  ...
  template <class charT, class Traits>
  std::basic_istream<charT, Traits>& operator()(std::basic_istream<charT, Traits>& in, unsigned& l) const {
    in >> l;
    ++l;
    return in;
  }
...
BOOST_AUTO_TEST_CASE(test_dynarray) {
  ...
  }
  {
    ...
    BOOST_CHECK(a[5] == 1);
    for (unsigned i = 0; i < 5; ++i) BOOST_CHECK(a.at(i) == 0);
  }
  const unsigned sz = 7;
  {
  ...
  BOOST_CHECK(search(a, a + sz, aa.begin(), aa.end())
              == a);  // really just tests begin,end as proper iterators
  ...
  for (unsigned i = 0; i < sz1; ++i) BOOST_CHECK(a1[i] == da[i]);
  db.removeMarked_nodestroy(rm2);
  BOOST_REQUIRE(db.size() == sz2);
  for (unsigned i = 0; i < sz2; ++i) BOOST_CHECK(a2[i] == db[i]);
  unsigned c = 0;
  ...
  for (unsigned i = 0; i < sz1; ++i) BOOST_CHECK(a1[i] == ea[i]);
  remove_marked_swap(eb, rm2);
  BOOST_REQUIRE(eb.size() == sz2);
  for (unsigned i = 0; i < sz2; ++i) BOOST_CHECK(a2[i] == eb[i]);
