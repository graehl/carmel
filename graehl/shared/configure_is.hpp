...
struct is_expr {
  ...
  is_expr const& flag(bool is_to = false) const { return init(is_to); }
  template <class V2>
  ...
  is_expr const& alias(bool = true) const { return *this; }
  template <class V2>
  is_expr const& eg(V2 const& eg) const {
    return *this;
  }
  ...
  is_expr const& allow_unrecognized(bool enable = true, bool warn = false,
  ...
  template <class V2>
  is_expr const& implicit(bool enable, V2 const& v2) const {
    return *this;
  }
  template <class V2>
  is_expr const& implicit(V2 const& v2) const {
    return *this;
  }
  is_expr const& defaulted(bool enable = true) const { return *this; }
  template <class V2>
  is_expr const& validate(V2 const& validator) const {
    return *this;
  }
};
