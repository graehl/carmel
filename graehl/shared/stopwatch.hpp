...
class stopwatch {
 public:
  enum timer_type { WALL_TIME, TOTAL_TIME, SYSTEM_TIME, USER_TIME, PAGEFAULTS, TYPE_MAX };
#define EACH_TIMER_TYPE(i) for (int i = WALL_TIME; i < TYPE_MAX; i++)
  explicit stopwatch(bool start_running = true) : running(start_running) { reset(start_running); }
  void start() {
    running = true;
#ifndef _WIN32
...
#undef EACH_TIMER_TYPE
  static bool valid_type(timer_type type) { return type >= WALL_TIME && type < TYPE_MAX; }
  template <class Time>
  ...
  static void measure_usage(rusage& r) {
    if (-1 == getrusage(RUSAGE_SELF, &r)) throw std::runtime_error("getrusage failed");
  }
#endif
  static void measure_wallclock(timeval& tv) {
    if (-1 == gettimeofday(&tv, 0)) throw std::runtime_error("gettimeofday failed");
  }
  ...
  double recent_total_time() const { return recent_user_time() + recent_system_time(); }
  ...
  void print(Ostream& os) const {
    os << '[' << total_time(WALL_TIME) << " wall sec, " << total_time(USER_TIME) << " user sec, "
       << total_time(SYSTEM_TIME) << " system sec, " << total_time(PAGEFAULTS) << " major page faults]";
  }
