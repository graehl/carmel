...
struct flag {
  ...
  friend inline bool latch(flag& once) { return once.first(); }
  void finish() { v = true; }
  void clear() { v = false; }
  /** \return if first() has already returned true (peeks without modifying state) */
  bool already() const { return v; }
  flag() : v() {}
  flag(bool v) : v(v) {}
...
struct counter {
  ...
  void print(std::ostream& out) const { out << v; }
  ...
  friend std::string type_string(counter const&) { return "count"; }  // TODO: ADL
};
