...
/*
  ...
  special form of compose: input string * transducer * output string (more efficient than compose.h (input
  string * transducer) * output string.

  NOTE: grapharc data field is an unsigned index into an arcs_table (which has the fstarc * and possibly
  ancillary counts).  for gibbs+cascade, link directly to cascade would be sufficient.

  believe/check: only one (composed) arc between any 2 states, because each state is
  cascade-state-tuple,i-pos,o-pos.  change in i,o pos determines label of arc.  with -a composition, that
  means only one cascade arc between any 2 deriv states.
...
// TODO: epsilon filter needed so training sums over subseqs of *e*:o and i:*e* don't care about order?  don't
// think so.
struct deriv_state {
  ...
  MEMBER_HASH
  bool operator!=(deriv_state const& r) const { return !(*this == r); }
  bool operator==(deriv_state const& r) const { return r.i == i && r.s == s && r.o == o; }
...
// above would suffice for boost::hash, but we're still 'flexible' and want to put in the same namespace as
// hashtable impl so it gets found by default, so:
...
struct arcs_table : public dynamic_array<arc_counts> {
  ...
  void operator()(unsigned s, FSTArc& a) {
    ...
    ac.set(s, &a,
           per_arc_prior ? global_prior + a.weight : global_prior);  // TODO: different prior per transducer
  }

#define ARCS_TABLE_EACH(i) \
  for (typename arcs_type::iterator i = this->begin(), end = this->end(); i != end; ++i)
#define ARCS_TABLE_EACH_C(i) \
  for (typename arcs_type::const_iterator i = this->begin(), end = this->end(); i != end; ++i)
...
struct wfst_io_index : boost::noncopyable {
  ...
  wfst_io_index(WFST const& x) : st(x.numStates()), i() {
    ((WFST&)x).visit_arcs(*this);
  }
  unsigned i;  // # arcs
  void operator()(unsigned src, FSTArc& a) { st[src][IOPair(a.in, a.out)].push_back(i++); }
};


// all the derivations for one in/out pair through WFST x
// note that the reverse graph and topo (cache_backward saves them) point to arcs directly via pointers, so
// serialization omits those (forces cache_backward=false on load), so they're rebuilt when needed
// containing derivations in a dynarray is fine since the lists used in graph states are movable, and
// state_to_id isn't kept around after compute, but even if it is, HashTable can also be moved.
struct derivations {
 private:
  ...
  void fill_id_to_state(id_to_state& f) {
    f.reinit(g.size());
    for (state_to_id::iterator i = id_of_state.begin(); i != id_of_state.end(); ++i) f[i->second] = i->first;
  }
 ...
 public:
  ...
  struct statistics {
    ...
    struct states_arcs {
      ...
      states_arcs() { clear(); }
      void clear() { states = arcs = 0; }
      void print(std::ostream& o) const { o << "(" << states << " states, " << arcs << " arcs)"; }
  ...
  bool empty() const { return no_goal; }
  state_id start() const { return 0; }
  state_id final() const {
    assert(!empty());
    return fin;
  }

  unsigned n_deriv_fsa_states() const { return g.size(); }
  ...
  */
  ...
  unsigned n_states() const { return g.size(); }
  ...
  struct weight_for {
    ...
    Weight operator()(GraphArc const& a) const { return t.ac(a).weight(); }
  };
...
#define ORANDPATH(x)  // std::cerr<<x
  ...
  struct pfor {
    fb_weights b;
    WeightFor const& wf;
    pfor(unsigned nst, unsigned fin, WeightFor const& wf)
        : b(nst)
        , wf(wf)  //
    {
      b[fin] = 1;
    ...
    double operator()(GraphArc const& a) const { return a.weight; }
  };
  ...
  void random_path(WeightFor const& wf, double power = 1.) {
    ...
    propagate_paths_in_order_wt(rg, reverse_order.begin(), reverse_order.end(),
                                pf.b);  // uses wf(a) from setwt
    free_order();
    free_reverse();
    unsigned s = 0;
    std::vector<bool> normed(
        nst, false);  // this is linear time; may actually slow down random choice vs. normalizing on fly
    while (s != fin) {  // fin should have no outgoing arcs if you want sampling to be sensible
      arcs_type& arcs = g[s].arcs;
      if (!normed[s]) {  // don't normalize (+compute probs for arcs) in states we don't visit, and save the
        // result if we revisit
        normed[s] = true;
        pf.global_normalize(arcs.val_begin(), arcs.val_end(), power);
      }
      GraphArc const& a = *choose_p(arcs.const_begin(), arcs.const_end(),
                                    pf);  // no empty states allowed that aren't final.
      // use the re-normalizing choose_p rather than choose_p01 to protect from rounding issues with
      // Weight::getReal sum to 1
  ...
  Weight collect_counts_gibbs(gibbs const& gi) {
    ...
    for (unsigned s = 0; s < nst; ++s) {
      ...
      for (arcs_type::const_iterator i = arcs.begin(), e = arcs.end(); i != e; ++i) {
        ...
        gi.choose_arc(a, arc_contrib / prob);  // TODO: VERY minor speedup: initialize b[fin] to 1/prob
        // instead of 1?  then no need to /prob.
      }
    }
    return prob;
  }
 ...
 private:
  derivations(derivations const& o)
      : in(o.in), out(o.out) {}  // similarly, this doesn't really copy the derivations; you need to compute()
  // after.  um, this would be bad if you used a vector rather than a list?
 public:
  // return true iff goal reached (some deriv exists)
  template <class Symbols>
  void init(Symbols const& in_, Symbols const& out_, double w = 1, unsigned line = 0,
            bool cache_backward_ = false) {
    in.set(in_);
    out.set(out_);
    weight = w;
    lineno = line;
  ...
  bool init_and_compute(WFST& x, wfst_io_index const& io, arcs_table const& atab, Symbols const& in_,
                        Symbols const& out_, double w = 1, unsigned line = 0, bool cache_backward_ = false,
                        bool prune_ = true, bool drop_names = true) {
    init(in_, out_, w, line, cache_backward_);
    return compute(x, io, atab, drop_names, prune_);
  }

  template <class arcs_table>
  bool compute(WFST& x, wfst_io_index const& io, arcs_table const& atab, bool drop_names = true,
               bool prune_ = true) {
#if DERIVPRUNE
    remove.clear();
#endif
    ...
    if (drop_names) id_of_state.clear();
    if (no_goal) {
      g.clear();
      return false;
    } else {
  ...
  struct reversed_graph {
    ...
    /// m[i] is set to val if state leads to i along a path in b, the reversed graph (i.e. in the orig. graph,
    /// something leads to "state".  you must set all m[i] !=val if they haven't already been visited by this
  ...
  void prune() {
    ...
    for (unsigned i = 0, N = remove.size(); i != N; ++i) {
      ...
      if (!r) dbg << "i=" << i << " " << (r ? '*' : '.') << " " << (r2 ? '*' : '.') << " " << i2s[i] << '\n';
#endif
      if (r != r2)
        Config::warn() << "state=" << i << " dfs=" << (r ? '*' : '.') << " build=" << (r2 ? '*' : '.')
                       << i2s[i] << '\n';
    }
    dbg << "\ngoal=" << goal << "\n";
#endif
 ...
 private:
  ...
  state_id derive(wfst_io_index const& io, arcs_table const& atab, deriv_state const& d) {
    const unsigned EPS = WFST::epsilon_index;
    state_id src = g.size();
    state_to_id::insert_result_type already = id_of_state.insert(d, src);
    if (!already.second) return already.first->second;  // return id if state was previously explored
    // new state = src - already added by insert above
    //        add(id_of_state,d,src); // NOTE: very important that we've added this before we start taking
    //        self-epsilons.
    g.push_back();
//        DBPC2("deriving",d);
#if DERIVPRUNE
  ...
  template <class arcs_table>
  bool add_arcs(wfst_io_index const& io, arcs_table const& atab, Sym s_in, Sym s_out, unsigned i_in,
                unsigned i_out, typename wfst_io_index::for_state const& fs, unsigned source) {
...
// note: use g[source] rather than caching the iterator, because recursion may invalidate any previously taken
// iterator
#if DERIVPRUNE
        if (!remove[dst])
#endif
        {  // FIXME: remove array isn't showing reachability to final (goal) state, so disabled.
          g[source].add_data_as(source, dst, a->weight.getReal(),
                                id);  // weight only used by gibbs init em prob
          reachgoal = true;
        }
      }
    return reachgoal;
  ...
  // because of pruning, there is not much gain in visiting from the final state in reversed order.  the
  // reverse of reverse_order is the forward order starting at start (0).  and the reverse of that is a valid
  // order for starting at finish and following reverse arcs (it's just that finish may not be the first thing
  // on the reverse_order, if pruning didn't happen, or if there were cycles

  void make_reverse() { r.init(g); }
  void free_reverse() {
    if (!cache_backward) r.clear();
  }

  void get_reverse() {
    if (!cache_backward) make_reverse();
  }
  ...
  void make_order() {
    ...
    if (rev.get_n_back_edges() > 0)
      Config::warn() << "Warning: at least one cycle in derivations for example (" << rev.get_n_back_edges()
                     << " back edges).  Forward/backward will miss some paths.\n";
  }

  void free_order() {
    if (!cache_backward) reverse_order.clear();
  }
  void get_order() {
    if (!cache_backward) make_order();
  }
};
