...
struct State {
  ...
  */
  ...
  // respects locked/tied arcs.  f(&weight) changes weight once per group or ungrouped arc, and never for
  // locked
  template <class F>
  struct modify_parameter_once : public F {
...
// if you distrust ht[key], I guess: //#define QUEERINDEX
  ...
  struct arc_adder {
    ...
    StateVector&
        states;  // only safe because we *know* that only shallow copies are made in moving lists around
    dynamic_array<I> bi;
    arc_adder(StateVector& states) : states(states), bi(states.size()) {}
