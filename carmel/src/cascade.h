...
/* TODO: for --crp with cascade of 1 transducer (set_trivial) make gibbs.cc set_gibbs_params work (set
 * cascade.cascade[0] to original xdcr)
 */
...
// track pointers to arcs in a composition cascade, and host shared lists of
// them, given a global index in the cascade, so that the groupId of the
// composed FSTArc gives a list.  note: composition of cascades gets its groupId
// set with this unique chain_id meaning a list of original input arcs
struct cascade_parameters {
  ...
  struct arcid_marker {
    ...
    void operator()(FSTArc const& a) { aid.add(&a, id++); }
  };

  template <class V>
  void visit_arcs(V& v) const {
    for (unsigned i = 0, n = cascade.size(); i < n; ++i) cascade[i]->visit_arcs_sourceless(v);
  }
  ...
  struct number_v {
    ...
    void operator()(unsigned src, FSTArc& a) { a.groupId = ++i; }
  };
  ...
  struct alpha_v {
    ...
    alpha_v(std::ostream& o, WFST::NormalizeMethod const& m)
        : o(o), prior(m.group == WFST::NONE ? -1 : m.add_count.getReal()) {}
    void operator()(FSTArc const& a) const { o << (a.isLocked() ? -1 : prior) << '\n'; }
  };
...
#include <graehl/shared/warning_pop.h>
  ...
  void fem_deriv(std::ostream& o, arcs_table<arc_counts> const& arcs, arcid_type const& aid,
                 derivations const& deriv) const {
  ...
  template <class arc_counts>
  void fem_deriv(std::ostream& o, arcs_table<arc_counts> const& arcs, arcid_type const& aid, backrefs& br,
                 GraphState* states, unsigned s, unsigned fin) const {
    ...
    for (List<GraphArc>::const_iterator l = st.const_begin(), end = st.const_end(); l != end; ++l) {
      ...
      bool nonleaf1
          = backdef
            || (p && (p->next
                      || mid));  // could postpone decision to use parens if !p (recursive flag passed in)
      if (nonleaf1) o << "(";
      graehl::word_spacer space;
      for (; p; p = p->next) o << space << aid[p->data];
      if (mid) {
        o << space;
  ...
  struct print_params_f {
    ...
    void operator()(FSTArc const& a) { o << a.weight << "\n"; }
  };
    ...
    void operator()(unsigned /*src*/, FSTArc& a) {
      if (!(i >> a.weight)) {
        throw std::runtime_error(
            "--load-fem-param file doesn't have enough params; make sure it was --fem-param saved for the "
            "same cascade");
      }
    }
  };
  ...
  WFST* pcomposed;
  // FIXME: to simplify, in trivial case and otherwise, composed should be passed in once so it can be added
  // to single chain (if trivial).  note: this simplification hasn't actually been done yet.
  void set_composed(WFST* c) {
    pcomposed = c;
    if (trivial) cascade.reinit(1, pcomposed);
  }
  ...
  // FIXME: per arc and global prior per cascade member (training of a cascade w/ prior has weird
  // interpretation now, want per-cascade as well or instead)
  ...
  typedef dynamic_array<chain_t>
      chains_t;  // // change w/ vector<arcid> ? so you know what component each arc came from
  chains_t chains;
  ...
  void operator()(unsigned s, FSTArc& a) { chains.at_grow(a.groupId) = pool.construct(&a, (chain_t)0); }
  ...
  unsigned size() const { return trivial ? 1 : cascade.size(); }

  void save_weights(saved_weights_t& save) const {
    for (cascade_t::const_iterator i = cascade.begin(), e = cascade.end(); i != e; ++i)
      (*i)->save_weights(save);
  }

  void restore_weights(saved_weights_t const& save) {
    saved_weights_t::const_iterator si = save.begin();
    for (cascade_t::iterator i = cascade.begin(), e = cascade.end(); i != e; ++i)
  ...
  void distribute_chain_counts(chain_t p, Weight counts) {
    for (; p; p = p->next) distribute_counts(*p->data, counts);
  }

  void distribute_counts(FSTArc& a, Weight counts) {
    if (!a.isLocked())
      // because locked arcs w/ weight other than 1 need to appear in chain, but can't have their weights
      // altered
      a.weight += counts;
  }
  ...
  void distribute_chain_id_counts(chain_id id, Weight counts) {
    assert(id < chains.size());
    // note: by construction, a cascade-composed fst will have no locked arcs; rather, it will refer to a
    // chain which references a locked arc
  ...
  struct arcs_table_distribute_counts {
    ...
    void operator()(unsigned /*source*/, FSTArc const& a) const {
      // note: using weight which is, after prep_new_weights, including the global prior. //FIXME: per-arc
      // prior in original transducers also
      cascade.distribute_chain_id_counts(a.groupId, a.weight);
    }
  };

  WFST& composed() const { return *pcomposed; }
...
#define EXCEPT_FOR_NONE(i) DO_FOR_NONE(i, != )
  ...
  // a save,modify,load is used instead of associating each arc in chains with which transducer it is, so as
  // to skip the NONE-normalized (weight-locked) ones.  only the NONE weights are saved/loaded, allowing the
  // others to change.
      ...
      , debug(debug)  //,tempnode(NULL,NULL)
  {
    ...
    // chains.push_back(0); // we don't mind using a 0 index since we don't use groupids at all when cascade
    // composing
  ...
  void normalize(WFST::NormalizeMethods const& methods) {
    ...
    for (unsigned i = 0, n = cascade.size(); i < n; ++i) cascade[i]->normalize(methods[i]);
  }
  ...
  void accumulate_chain(Weight& w, chain_t p) {
    for (; p; p = p->next) w *= p->data->weight;
  }

  // todo: using structure of composition, could perhaps use the shared structure to reduce number of
  // accumulates (linked list by index might be tricky but possible, toposort dependencies then go?
  void calculate_chain_weights() {
    chain_weights.clear();
    chain_weights.resize(chains.size(), Weight::ONE());
    for (unsigned i = 0, e = chains.size(); i != e; ++i) {
      accumulate_chain(chain_weights[i],
                       chains[i]);  // recall, nil chain will keep the default ONE as it's an empty list
    }
  }

  void print(std::ostream& o, bool cascade = true, bool chains = true) {
    if (cascade) print_cascade(o);
    if (chains) print_chains(o);
  }
  ...
  void print_chains(std::ostream& o, bool weights = true) {
    o << "Composed chains:\n";
    for (unsigned i = 0, e = chains.size(); i != e; ++i) print_chain(o, i, weights);
  }

  struct param_writer {
    template <class Ch, class Tr, class value_type>
    std::basic_ostream<Ch, Tr>& operator()(std::basic_ostream<Ch, Tr>& o, const value_type& l) const {
      return o << '@' << l << ':' << *l;
    }
  };
  ...
  void update() {
    if (trivial) return;
    if (debug & DEBUG_COMPOSED) Config::debug() << "composed pre:\n" << composed() << std::endl;
    ...
    if (debug & DEBUG_COMPOSED) Config::debug() << "composed post:\n" << composed() << std::endl;
  }


  void clear_groups() {
    for (unsigned i = 0, e = cascade.size(); i != e; ++i) cascade[i]->clear_groups();
  }
  ...
  void prepare_compose(
      bool right_assoc)  // shorthand for left->right.  right_assoc -> first is not chain, second is
  {
    if (right_assoc)
  ...
  chain_id record1(FSTArc const* e) { return record_eps(const_cast<param>(e), is_chain[0]); }

  chain_id record2(FSTArc const* e) { return record_eps(const_cast<param>(e), is_chain[1]); }
  ...
  inline chain_t cons(param a, param b) { return cons(a, cons(b)); }
  ...
  // FIXME: untested, since we only compose strictly left or right assoc. - maybe better sharing w/ a tree
  // cons structure (type flag indicates leaf? if you allow arbitrary assoc.
  inline chain_t cons(chain_t a, chain_t b) {
    for (; a; a = a->next) b = cons(a->data, b);
    return b;
  }

  chain_t operator[](FSTArc const* arc) const { return operator[](*arc); }

  chain_t operator[](FSTArc const& arc) const {
    /* // doesn't work because these are persisted in sample (acpaths).  would need memory pool; instead just
       initialize chains explicitly w/ set_trivial_gibbs_chains
       if (trivial) {
  ...
  static inline bool is_locked_1(param e) { return e->isLocked() && e->weight.isOne(); }

  chain_id locked_1_groupid() { return trivial ? WFST::locked_group : nil_chain; }

  inline chain_id original_id(param e) { return is_locked_1(e) ? nil_chain : e->groupId; }
  ...
  chain_id record(param a, param b) {
    ...
    // TODO: in theory a and/or b could be epsilons from earlier, and although we bothered to collapse the
    // epsilons with epsilon_chains hash earlier, we'd be making redundant pairs.  a similar hash on pairs
    // (a,b) could ensure that same-bracketing cons structures are stored only once
    chain_t v = cons_chain(a, b);
    if (!v) return nil_chain;
    chain_id ret = chains.size();
    chains.push_back(v);
    return ret;
  }

  // used to find now-defunct (after final-state-reachability reduction) arcs' chains and remove them.  should
  // lead to some slight per-iteration speedup in calculate_weights and a little memory saving.
  // the arc visitation skips locked arcs which come up e.g. with multiple finals during composition w.r.t
  // epsilon-filter state.
  struct remap_chains {
    indices_after_removing newids;
    fixed_array<bool> remove;

    remap_chains(unsigned n, unsigned nil_chain) : remove(true, n) { remove[nil_chain] = false; }
    ...
    void rewrite_arcs(WFST& w) { w.visit_arcs(*this); }
  ...
  void done_composing(WFST* composed, bool compress_removed_arcs = false) {
    ...
    if (trivial) return;
    epsilon_chains.clear();
    if (compress_removed_arcs) compress_chains();
  }
