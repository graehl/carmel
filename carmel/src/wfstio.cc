...
void WFST::output_format(bool* flags, std::ostream* fstout) {
  if (fstout) {
    ...
    if (flags[(unsigned)'D']) Weight::out_never_log(*fstout);
    if (flags[(unsigned)'J'])
      out_arc_full(*fstout);
    else
  ...
  if (flags[(unsigned)'D']) Weight::default_never_log();
...
// Generate a permutation lattice for a given string
{
  ...
  while (line) {
    ...
    (void)out.indexOf(symbol);
    symbols.push_back(symbolInNumber);
    if (maxSymbolNumber < symbolInNumber) maxSymbolNumber = symbolInNumber;
  }
  if (permuteNumbers) {
    push_back(states); /* final state*/
          ...
          if (!((k / temp) % 2) && !taken[words[l].c_str()]) {
            if (isNonNegInt(words[l].c_str())) {
              ...
              for (unsigned i = 1; i < words[l].length() - 1; i++) {
                string s("\"\"\"");
                s[1] = words[l][i];
                if (NULL == in.find(const_cast<char*>(s.c_str()))
                    || NULL == out.find(const_cast<char*>(s.c_str()))) {
                  ...
                  if (i == words[l].length() - 2)
                    ...
                    to_state = ++temp_final;
                  if (from_state >= states.size()) states.resize(from_state);
                  //              std::cerr << "adding arc (from:" << from_state << ", to:"<<to_state <<",
                  //              in/out:"<<s <<"("<<in.indexOf(const_cast<char *>(s.c_str()))<<"))\n";
                  states[from_state].addArc(FSTArc(in.indexOf(const_cast<char*>(s.c_str())),
                                                   in.indexOf(const_cast<char*>(s.c_str())), to_state, 1.0));
                  from_state = to_state;
                }
              }
            } else {
              if (NULL == in.find(const_cast<char*>(words[l].c_str()))
                  || NULL == out.find(const_cast<char*>(words[l].c_str()))) {
                std::cerr
                    << "Error in constructing a permutation lattice!! didn't find symbol in the alphabet "
                    << words[l] << '\n';
              } else {
                //              std::cerr << "adding arc (from:" << k << ", to:"<<k+temp <<",
                //              in/out:"<<words[l].c_str() << '\n';
                states[k].addArc(FSTArc(in.indexOf(const_cast<char*>(words[l].c_str())),
                                        out.indexOf(const_cast<char*>(words[l].c_str())), k + temp, 1.0));
              }
            }
...
unsigned WFST::getStateIndex(const char* buf) {
  ...
  if (!named_states) {
    ...
    if (*buf && *scanend != '\0') {
      Config::warn() << "Since intial state was a number, expected an integer state index, but got: " << buf
                     << std::endl
                     << "\t(-K command line option should not be used if states are named)" << std::endl;
      return -1;
    } else {
...
#define GETBUF(buf) REQUIRE(e = getString(istr, buf))
            ...
            if (ENDIOW) {  // ... weight) or ... symbol)
              // FIXME: document: this means that even though unquoted symbols are supported, you must quote
              // any symbol that can parse into a weight, e.g. -10ln, e^3, 10^-3, 0.1
...
#undef GETBUF
    ...
    if (!named_states) {
      if (!(final < size()))
        goto INVALID;  // whoops, this can never happen because of getStateIndex creating the (empty) state
...
INVALID:
  ...
  if (named_states) finalName.kill();
  invalidate();
  return 0;
}
...
void WFST::writeGraphViz(ostream& os) {
  ...
  for (unsigned s = 0; s < numStates(); s++) {
    for (List<FSTArc>::const_iterator a = states[s].arcs.const_begin(), end = states[s].arcs.const_end();
         a != end; ++a) {
      os << newl;
...
void WFST::writeLegible(ostream& os, bool include_zero) {
  ...
  for (i = 0; i < numStates(); i++) {
    if (!onearc) os << "\n(" << stateName(i);
    for (List<FSTArc>::const_iterator a = states[i].arcs.const_begin(), end = states[i].arcs.const_end();
         a != end; ++a) {
