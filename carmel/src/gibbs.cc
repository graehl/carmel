...
struct carmel_gibbs : public gibbs_base {
  ...
  carmel_gibbs(WFST& composed, cascade_parameters& cascade, training_corpus& corpus,
               WFST::NormalizeMethods& methods, WFST::train_opts const& topt, gibbs_opts const& gopt,
               WFST::path_print const& printer, WFST::saved_weights_t* init_sample_weights = NULL)
    ...
    set_cascadei();
    if (init_sample_weights && !cascade.trivial) composed.restore_weights(*init_sample_weights);
  ...
  struct print_counts_f {
    ...
    bool final;
    bool show_metanorm;
    print_counts_f(carmel_gibbs& g, unsigned from, unsigned to, bool final = false, bool show_metanorm = true)
        : g(g), from(from), to(to), i(), final(final), show_metanorm(show_metanorm) {}
    bool operator()(FSTArc const& a) {
      unsigned gi = a.groupId;
      //            if (i>=from && i<to)
      if (gi >= from && gi < to) g.print_count(gi, g, final, show_metanorm);
      //            return (++i<to);
      return true;
    }
  };
  ...
  void operator()(unsigned src, FSTArc& a) const { a.weight.setReal(gibbs_base::final_prob(a.groupId)); }
  ...
  struct add_gibbs_nonorm {
    ...
    add_gibbs_nonorm(carmel_gibbs& self, bool add_arcs, bool add_source)
        : self(self), add_arcs(add_arcs), add_source(add_source) {}
    void operator()(unsigned src, FSTArc& a) { self.add_locked(src, a, add_arcs, add_source); }
  };
  ...
  // compute the prior pseudocount gps[i].prior as alpha*M*p0 where M is the size of the normgroup and p0 is
  // the (normalized) value on the arc.  if uniformp0, then pseudocount is just alpha (same as uniform p0)
  // return next free normgroup id, start at normidbase
  unsigned add_gibbs_params(unsigned /*normgroup start*/ id, WFST& w, WFST::NormalizeMethod const& nm,
                            bool add_arcs = false, bool add_source = false) {
...
#include <graehl/shared/warning_pop.h>
        ...
        */
        ...
        ++id;
        if (pgroup == WFST::LOCAL) prior_scale.finish_scalegroup();
      }
      if (pgroup == WFST::SINGLE) prior_scale.finish_scalegroup();
    }
  ...
  // same order as add_gibbs_params ; important: conditional norm will give different param order than
  // --fem-param etc.
  template <class V>
...
#include <graehl/shared/warning_pop.h>
  ...
  typedef cascade_parameters::chain_t param_list;  // a composed arc has a derivs.arcs entry eventually
                                                   // linking to a list of original cascade arcs making it up
                                                   // (will have either 0 or 1 items in a -a composition,
                                                   // however; we could simplify by forcing -a)
  cached_derivs<arc_counts_base> derivs;  // derivs.arcs is an arcs_table<arc_counts_base>.
                                          // derivs.arcs.ac(GraphArc a) gives the arc_counts_base for a deriv
                                          // arc a.  arc_counts_base just has FSTArc *arc (unlike EM
                                          // arc_counts which also has counts etc.)
  FSTArc* composed_arc(GraphArc const& a) const { return derivs.arcs.ac(a).arc; }
  param_list ac(GraphArc const& a) const { return cascade[composed_arc(a)]; }

  typedef WFST::saved_weights_t saved_weights_t;
  WFST::saved_weights_t* init_sample_weights;  // FIXME: should probably go by the wayside; not very
                                               // productive since burnin should give conditional indep.
                                               // anyway; if we keep it, then store as a double array instead
                                               // and modify p_init to reference that.
  ...
  WFST& wfst_ci(unsigned ci) const { return *cascade.cascade[ci]; }
  WFST& wfst_for(unsigned parami) const { return wfst_ci(cascadei_for(parami)); }
  ...
  void paths(block_t const& p, unsigned a, unsigned b, cascade_paths& r) {
    assert(b <= r.size());
    for (unsigned i = a; i < b; ++i) r[i].clear();
    for (block_t::const_iterator i = p.begin(), e = p.end(); i != e; ++i) {
      unsigned parami = *i;
      unsigned ci = cascadei_for(parami);
      if (ci >= a && ci < b) r[ci].push_back(parami);
    }
  }
  void print_sample(blocks_t const& sample, unsigned a, unsigned b, casc const& c) {
    ...
    if (!(b > a && a < c.size())) {
      if (b != 0)
        Config::warn() << "--print-from,-to gibbs [" << a << "," << b << ") is out of range for " << c.size()
                       << " input transducers.\n";
      return;
    }
  ...
  void print_path(block_t const& p, unsigned a, unsigned b, casc const& c) {
    ...
    paths(p, a, b, ps);
    for (unsigned i = a; i < b; ++i) print_cascade_path(*c[i], ps[i]);
  ...
  void print_sample(blocks_t const& sample) { print_sample(sample, gopt.print_from, gopt.print_to); }
  void set_cascadei() {
...
#define OUTGIBBS3(x)  // OUTGIBBS(x)
  double block_weight(unsigned block) { return derivs.derivs[block].weight; }
  ...
  void resample_block(unsigned block) {
    ...
    blockp = &b.id;
    if (gopt.expectation) {
      // no support for init_prob yet.  (different initial weights than the base model for computing first
      // iter expectation).  doesn't seem useful anyway.  gopt.random_start happens after anyway
      blockd = &b;
      b.prob = d.collect_counts_gibbs(*this);
    } else {
      if (init_prob)  // if iteration==0
        d.random_path(p_init(*this), power);  // because init sample distribution may be different from p0
                                              // e.g. from EM.  this also means we aren't using cache to
                                              // generate first sample at all
      else
...
#define DGIBBS2(x)  // x
  ...
  struct p_init {
    ...
    void choose_arc(GraphArc const& a) const { return c.choose_arc(a); }
  };
  // for resample block:
  ...
  void init_run(unsigned r) {
    ...
    if (init_prob && init_sample_weights && cascade.trivial) composed.restore_weights(*init_sample_weights);
  }
...
void WFST::train_gibbs(cascade_parameters& cascade, training_corpus& corpus, NormalizeMethods& methods,
                       train_opts const& topt, gibbs_opts const& gopt1, path_print const& printer,
                       double min_prior) {
  ...
  for (NormalizeMethods::iterator i = methods.begin(), e = methods.end(); i != e; ++i) {
    if (i->add_count <= 0) {
      Config::warn()
          << "Gibbs sampling requires positive --priors for base model / initial sample.  Setting to "
          << min_prior << "\n";
  ...
  //    also, this allows the dirichlet to be specified per-normgroup with different effective
  //    concentration,alpha
  if (restore) cascade.save_weights(saved);
  if (em || gopt.init_from_p0) {
    NormalizeMethods m2 = methods;
    for (NormalizeMethods::iterator i = m2.begin(), e = m2.end(); i != e; ++i) i->add_count = 0;
    if (em) {
      train_opts t2 = topt;
  ...
  // restore old weights; can't do gibbs init before em because groupId gets overwritten; em needs that id
  // from composition
